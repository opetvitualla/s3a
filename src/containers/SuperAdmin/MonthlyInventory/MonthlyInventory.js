...
class MonthlyInventory extends Component {
    constructor(props) {
        ...
        this.toggleModal_approve = this.toggleModal_approve.bind(this);
    ...
    Raw_material = async (fromDate, toDate, material) => {
        ...
        let url = Config.base_url + 'testing_janu/get_to_approve_inventory/';
        let response = await axios.post(url, qs.stringify(formData));
        let temp_data = [];
        ...
        if (response.data.status == 'success') {
            const m = response.data.data.map((key) => {
                console.log(key)
                let groupBtn = [
                    { title: "Edit", icon: "ion-edit", color: "info", function: () => this.editForm(key.pk_pending_inventory_id) },
                ];
                // let recInv = {
                ...
                // }
                let tbldata = {
                    pk_pending_inventory_id: "INV-" + key.pk_pending_inventory_id,
                    recorded_by: key.first_name + " " + key.last_name,
                    inventory_type: key.inventory_type,
                    start_date: key.start_date,
                    end_date: key.end_date,
                    date_recorded: key.date_recorded,
                    status: key.inv_status,
                    // action: <button type="button" data-id={key.pk_pending_inventory_id} onClick={(e) => this.toggleModal_approve()} className="float-center btn btn-info real-btn btn btn-secondary">Approve</button>,
                    action: this.checkPOstatus(key.inv_status, key.pk_pending_inventory_id),
                }
                temp_data.push(tbldata);



            });
            console.log(temp_data, "test")
            this.setState({ data_monthly: temp_data });
        }
    }

    checkPOstatus = (stat, inv_id) => {
        let groupBtn = [
            { title: "Approve", icon: "ion-checkmark", color: "primary", disabled: false, function: () => this.toggleModal_approve(inv_id) },
            { title: "View", icon: "ion-eye", color: "warning", disabled: false, function: () => this.view_pending_data(inv_id) },
        ];
        let groupBtn2 = [
    ...
    view_pending_data = async (inv_id) => {

        let url = Config.base_url + 'testing_janu/get_monthly_data/' + inv_id;
        ...
        if (response.data.status == "success") {
            ...
            this.setState({ inv_table_data: datas })
        }
    }
    ...
    updatePendingInventory = async (e) => {
        e.preventDefault();

        let formData = {
            inv_id: this.state.mon_inv_id
        }

        let url = Config.base_url + 'testing_janu/approve_inventory_request/';
        let response = await axios.post(url, qs.stringify(formData));
    ...
    toggleModal_approve = async (inv_id) => {
        await this.setState({
    ...
    render() {
        ...
        $(document).ready(function () {
            ...
            str += "</tr><tr>";
            str += "<th>Inventory ID</th><th>Recorded By</th><th>Type</th><th>Start Date</th><th>End Date</th><th>Date Recorded</th><th>Status</th><th>Action</th>";
            str += "</tr>";
        ...
        return (
            <AUX>
                ...
                <Modal isOpen={this.state.AppModal}>
                    <ModalHeader toggle={() => this.setState({ AppModal: false })} ></ModalHeader>
                    <ModalBody>
                        <p>Are you sure you want to approve this inventory?</p>
                    </ModalBody>
                    <ModalFooter>
                        <Button color="primary" className="btn btn-primary waves-effect" onClick={this.updatePendingInventory}>Yes</Button>
                        <Button color="secondary" onClick={() => this.setState({ AppModal: false })}>Cancel</Button>
                    </ModalFooter>
                </Modal>

                <Modal size="xl" isOpen={this.state.viewModal} toggle={this.viewModalToggle} className={this.props.className}>
                    <ModalHeader toggle={this.viewModalToggle}> View Inventory Details</ModalHeader>
                    ...
                    <ModalBody>
                        ...
                        <div className="row">
                            <div className="col-12">
                                <div className="p-2">
                                    <h3 className="font-16"><strong>Inventory Summary</strong></h3>
                                </div>
                                <table className="table ">
                                    <thead>
                                        <tr>
                                            <td className="text-center"><strong>Items</strong></td>
                                            <td className="text-center"><strong>Packging</strong></td>
                                            <td className="text-right"><strong>BEG.Balance</strong></td>
                                            <td className="text-right"><strong>Consumed</strong></td>
                                            <td className="text-right"><strong>Add</strong></td>
                                            <td className="text-right"><strong>Boxes</strong></td>
                                            <td className="text-right"><strong>End Bal.</strong></td>
                                            <td className="text-right"><strong>Boxes</strong></td>
                                            <td className="text-right"><strong>End Bal.</strong></td>
                                            <td className="text-right"><strong>Variance</strong></td>
                                            <td className="text-right"><strong>Note</strong></td>
                                        </tr>
                                    </thead>
                                    <tbody>

                                        {
                                            this.state.inv_table_data.map((key, index) => {

                                                let boxes_rec = parseInt(key.mat_end / key.packaging_val);
                                                let boxes_phy = (key.physical_boxes != null) ? key.physical_boxes : 0;
                                                let end_phy = (key.physical_endbals != null) ? key.physical_endbals : 0
                                                let variance = (key.physical_endbals != null) ? key.physical_endbals - key.mat_end : 0;

                                                return (
                                                    <tr>
                                                        <td className="text-center">{key.mat_name}</td>
                                                        <td className="text-center">{key.packaging_type}</td>
                                                        <td className="text-center">{key.beginning_bal}</td>
                                                        <td className="text-center">{key.mat_out}</td>
                                                        <td className="text-center">{key.mat_in}</td>
                                                        <td className="text-center">{boxes_rec}</td>
                                                        <td className="text-center">{key.mat_end}</td>
                                                        <td className="text-center">{boxes_phy}</td>
                                                        <td className="text-center">{end_phy}</td>
